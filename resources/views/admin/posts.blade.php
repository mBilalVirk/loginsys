                    ...
                    @foreach ($posts as $user)
                        @foreach ($user->posts as $post)
                            <tr>
                                ...
                                <td>
                                    @if ($post->photo)
                                        <img src="{{ asset($post->photo) }}" class="post-image"
                                            style="width:30px; margin-top:10px; border-radius:8px; cursor:pointer;"
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#viewComments{{ $post->id }}">
                                        View : {{ $post->comments->count() }}
                                    </button>

                                    <!-- Comments Modal -->
                                    <div class="modal fade" id="viewComments{{ $post->id }}" tabindex="-1">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('editpost', $post->id) }}" method="POST"
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="content" required class="form-control" style="height: 200px;">{{ $post->content }}</textarea>
                                ...
                                <td>
                                    <form action="{{ route('delpost', $post->id) }}" method="POST"
                                        onsubmit="return confirm('Are you sure you want to delete this post?');">
                                        @csrf
                                        @method('DELETE')
